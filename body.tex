...
Subsequently, we will use the Forward Global Calibration Method \citep[FGCM;][]{2018AJ....155...41B}, an uber-calibration method, to derive photometric zeropoints for the LSST data and achieve a photometric precision of better than 5 mmag.
...
\begin{itemize}
    ...
    This is achieved by using \textit{Gaia} Data Release 3 (\emph{Gaia}-DR3; \citealt{2023A&A...674A...1G}) as the basis for the objects in our catalog, with the full high-quality position and proper motion measurements from \textit{Gaia}-DR3 included.
\end{itemize}

Previous catalogs such as ATLAS-REFCAT2 \citep{2018ApJ...867..105T} provide all-sky coverage, but are not sufficiently deep to provide the required source density at magnitudes probed by LSST images.
PANSTARRS-1\citep[PS1;][]{2016arXiv161205560C} provides sufficient depth in \emph{grizy}-bands but does not have \emph{u}-band, and its sky coverage is limited to declinations above $\delta = -30$ deg.
...
\begin{enumerate}
    \item DES Y6 Calibration Stars \citep[Section \ref{sec:des};][]{2023arXiv230501695R}: calibrated photometric reference stars from Dark Energy Survey (DES) year 6 data.
    \item \emph{Gaia}-XP Synthetic Magnitudes \citep[Section \ref{sec:gaiaxp}][]{2023A&A...674A..33G}: synthetic photometry generated from \emph{Gaia} spectrophotometry using the \texttt{GaiaXPy} package.
    \item PS1 \citep[Section \ref{sec:ps1};][]{2016arXiv161205560C}: photometry from the Pan-STARRS1 3pi survey, containing sources at declinations above -30 degrees.
    \item SkyMapper \citep[Section \ref{sec:skymapper};][]{2019PASA...36...33O}: southern-sky photometry from Data Release 2 of the SkyMapper survey.
    \item VST \citep[Section \ref{sec:vst};][]{2015MNRAS.451.4238S}: Data Release 4 of data from VST ATLAS survey.
\end{enumerate}

For \textit{u}-band we use:
\begin{itemize}
    \item SDSS Standard Stars \citep[Section \ref{sec:sdss};][]{2020ApJS..249....3A}: stars from Sloan Digital Sky Survey (SDSS) Data Release 16 with signal-to-noise greater than 5.
    \item \emph{Gaia}-XP Synthetic Magnitudes \citep[Section \ref{sec:gaiaxp}][]{2023A&A...674A..33G}: synthetic photometry generated from \emph{Gaia} spectrophotometry using the \texttt{GaiaXPy} package.
...
These transformations were derived using the baseline v1.9 LSST total transmissions available at \url{https://github.com/lsst/throughputs/tree/1.9/baseline}, as well as the DES throguhputs from \citet{2021ApJS..255...20A} accessable at \url{https://noirlab.edu/science/programs/ctio/filters/Dark-Energy-Camera}.
The transfomations are found using the FGCM stellar templates \citep{2018AJ....155...41B}. 
These templates are a repackaged version of the main sequence stellar library presented in \citet{2012ascl.soft08007K}.
...
\begin{figure}
    ...
    Note that the \emph{Gaia}-XP synthetic photometry uses the raw values not the standardized values from \citet{2023A&A...674A..33G}.}
...
\label{sec:des}
Data is described in \citep{2023arXiv230501695R}.
...
\label{sec:gaiaxp}
As inputs to \monster we use synthetic photometry derived from the \emph{Gaia}-XP spectra \citep{2023A&A...674A..33G}. 
...
For more information see \url{http://panstarrs.stsci.edu} and \citet{2016arXiv161205560C}.

\subsection{SkyMapper}
\label{sec:skymapper}
For \monster we use DR2 of the SkyMapper catalog \citep{2019PASA...36...33O} downloaded from \url{https://skymapper.anu.edu.au/_data/DR2/}. 
